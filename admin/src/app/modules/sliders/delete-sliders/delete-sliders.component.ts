...
export class DeleteSlidersComponent {
  @Input() slider: any;
  @Output() SliderDelete: EventEmitter<any> = new EventEmitter();
  isLoading: any;
  ...
  delete() {
    this.sliderService.deleteSlider(this.slider.id).subscribe((res: any) => {
      this.SliderDelete.emit({ message: 200 });
      this.modal.close();
    });
  }
}
