...
<body>
    ...
    {{-- <button type="button" class="btn btn-light-primary font-weight-bold" onclick="window.print();">Imprimir
        Pedido</button> --}}
    <div class="web-container">
                ...
                <tbody>
                    <tr>
                        <td style="padding: 0 !important;border-bottom:none;">
                            <img style="height: 75px;background:black;border-radius:5px"
                                src="{{public_path("logo.png")}}">
                        </td>
                        <td style=" padding: 0 !important;border-bottom:none;">
                            <br>
                            <small>Tel. 912123456</small>
                            <br>
                            <small> Mediashop <br> Calle Ramón Arias nº 25 Málaga, 20134 <br>
                                Málaga, España </small>
                        </td>
                    </tr>
                </tbody>
        ...
        <table class="invoice-info-container">
            <tr>
                <td>
                    N° VENTA: <strong>#{{ $sale->code_transaction }}</strong>
                </td>
                <td>
            ...
            @if ($sale->n_contrato_fisico)
                <tr>
                    <td>
                        CONTRATO FISICO: <strong>#{{ $sale->n_contrato_fisico }}</strong>
                    </td>
                    <td></td>
                </tr>
            @endif
            <tr>
                ...
                <td class="">
                    ...
                    <br />
                    EMAIL: {{ $sale->user->name . ' ' . $sale->user->surname }}
                    <br />
            ...
            <tr>
                <td>
                    <b>Datos de Envío: </b>
                    <br />
                    DIRECCIÓN: <strong>{{$sale->sale_addres?->street_address}}</strong>
                    <br />
                    EMPRESA: <strong>{{$sale->sale_addres?->company}}</strong>
                    <br />
                    CIUDAD/REGIÓN: <strong>{{$sale->sale_addres?->city}},
                        {{$sale->sale_addres?->country_region}}</strong>
                </td>
                <td></td>
            </tr>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            @if ($item->product_variation)
                                ...
                                {{$item->product_variation->property ? $item->product_variation->property->name : $item->product_variation->property->value_add}}
                                @if ($item->product_variation->variation_parent)
                                    <br />
                                    <strong>{{$item->product_variation->variation_parent->attribute->name}}:</strong>
                                    {{$item->product_variation->variation_parent->property ? $item->product_variation->variation_parent->property->name : $item->product_variation->variation_parent->property->value_add}}

                                @endif
                            @endif
                            @if ($item->discount > 0)
                                <br />
                                <strong> Descuento: - {{$item->discount}}
                                    {{$item->typpe_discount == 1 ? '%' : $item->currency}}</strong>
                                @if ($item->type_campaign)
                                    Campaña: {{ $item->code_discount }}
                                @else
                                    Cupón: {{ $item->code_coupon }}
                                @endif
                            @endif
                        </td>
